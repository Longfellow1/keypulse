...
**关键改变 v2**：
1. **主题标题改为"结果名词化"不是"动作动词"**。
   - ❌ 坏例：「KeyPulse 从实验工具切换为日用产品」（动词短语）
   - ✅ 好例：「KeyPulse 日用化完成」或「KeyPulse 生产就绪」（名词化结果）
2. **删除 TL;DR 模板拼接指令。** TL;DR 由渲染层负责，prompt 不干预。
3. **narrative 中的判断句不能是"这件事的结果已进入可继续迭代阶段"这类流水套话**。要么回答"为什么选 X 不选 Y"，要么说"如果不做 X 会发生什么"，要么明确引用上周状态对比。
4. **"这意味着" 改为可选**。只有当你能用 (a)(b)(c)(d) 之一支撑时才写。否则**就不输出这一行**。没有 "meaning" 字段就用空字符串，渲染层见空字符串不输出"→ 这意味着"这一行。
5. **禁止写"把 X 的结果整理成下周可复现的执行入口"这类冗长套话**——下周锚点由 L6 输出，narrative 里不需要。
