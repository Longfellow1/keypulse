...
**同类软降权机制**：
- 新候选生成时跑同类检测（embedding 相似度 or 标题语义聚类，先用简单字符串重合度兜底）
- 命中同类时旧版 `score *= 0.3`，**不删文件**，留 `~/.keypulse/skills/_archive/` 软出局通道
- 用户可手动恢复（mv 回主目录）
...
## 3.5 挖矿算法 — 多维信号融合 + 跨日阈值

**核心设计**：skill 候选不是单次发言，而是**跨日重复模式 + 多维信号的加权融合**。一次性发言不算金矿。

### 三路信号

1. **语言信号** — chunk 里出现「我相信 / 我倾向 / 我宁可 / 不能 X 否则 Y」+ 主语第一人称。信号强度：直接信仰宣言 > 条件判断 > 行为倾向
2. **行为重复** — 同一类决策点在多个 chunk 里重复出现（不同上下文，同样的判断)
3. **主题聚类** — 语义聚类落到同一簇（cross-chunk embedding 相似度）

### 融合 = 加权打分

三路各给分，加权求和。hello world 阶段先 `1:1:1` 跑通，后续 eval 调权。

### 跨日 ≥3 次阈值

**候选必须在 ≥3 个不同 calendar day 出现才能进周报推荐**。这是反「单事件浮夸假设」的核心闸门。
...
| 维度 | 设计 |
|---|---|
| **频率** | 每周一次（周报渲染时，plain + exec 两个 style 都触发），用户也可手动 `keypulse skill propose` 强触发 |
| **触发流程** | weekly note 渲染时自动跑挖矿，取 top N（N=3）写进「方法论」节 |
| **披露形态** | 候选直接落 `~/.keypulse/skills/{slug}.md`，文件名是 KeyPulse 给的初稿。**用户改文件名 / 改内容就是命名权矫正**（异步、无勾选界面） |
| **兜底** | 当周没达到阈值（跨日 ≥3 次）的候选，整节就空着，不强凑 |
...
| **skill eval（4 维度）** | 多周 daily/weekly → N 个 skill 候选 | 见 14.2 四维度框架 | 待 hello world 跑通后建立 |

### 14.2 Skill Eval — 4 维度框架

| 维度 | 指标 | 黄金集构造方式 |
|---|---|---|
| **1. 金矿识别准确率** | precision / recall vs 人工标注 | 用户对历史 chunk 手工标「是/否金矿」 |
| **2. 命名灵魂率** | 1 - (用户改文件名比例) | 跑一周后统计 `~/.keypulse/skills/` 改名率 |
| **3. 假设语气合规率** | LLM judge: 是否用假设句式 | 抽样 + LLM judge |
| **4. 同类降权准确率** | 同类对被检出比例 vs 人工标注 | 注入同类对 + 干扰对 test set |

**Fail case 一等公民**：每个 fail case（单次发言被识别为金矿、命名套话被用户改、同类没检测出来）单独建 markdown 案件进墙。
